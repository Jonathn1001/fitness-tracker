...
type DS = Pick<
  SessionDetail,
  'id' | 'date' | 'templateDay' | 'sessionSets' | 'sessionRounds'
>
...
function buildTrend(sessions: DS[], exerciseName: string) {
  return sessions
    ...
    .flatMap((s) => {
      const top = s.sessionSets
        .filter((ss) => ss.exercise.name === exerciseName)
        .map((ss) => ss.weightKg)
...
function buildKbQuality(sessions: DS[]) {
  return sessions
    ...
    .flatMap((s) => {
      ...
      return rated.length
        ? [rated.reduce((a, b) => a + (b.qualityRating ?? 0), 0) / rated.length]
        : []
    })
}
...
export function ProgressPage() {
  ...
  const maxVol = Math.max(
    ...MUSCLE_GROUPS.map((mg) => muscleVol.get(mg) ?? 0),
    1,
  )

  const prs = [
    bench.length
      ? { name: 'Bench Press', v: `${Math.max(...bench)} kg` }
      : null,
    squat.length ? { name: 'Back Squat', v: `${Math.max(...squat)} kg` } : null,
    kb.length
      ? { name: 'Round quality', v: `${Math.max(...kb).toFixed(1)} / 5` }
      : null,
  ...
  return (
    <>
      ...
      <div className="seg">
        <button
          className={metric === 'bench' ? 'on' : ''}
          onClick={() => setMetric('bench')}
        >
          Bench
        </button>
        <button
          className={metric === 'squat' ? 'on' : ''}
          onClick={() => setMetric('squat')}
        >
          Squat
        </button>
        <button
          className={metric === 'kb' ? 'on' : ''}
      ...
      <div className="card chart-card">
        <div className="chart-head">
          ...
          {s.data.length >= 2 && (
            <div className="chart-delta" style={{ color: s.color }}>
              <Icon name="trend" size={14} /> {Number(delta) >= 0 ? '+' : ''}
              {delta}
              {s.unit}
            </div>
          )}
        ...
        <MiniChart
          data={s.data}
          color={s.color === 'var(--accent)' ? '#C8FF3E' : '#FF7A59'}
        />
        {s.data.length >= 2 && (
                ...
                <div className="cov-bar">
                  <div
                    ...
                    style={{
                      width: `${pct * 100}%`,
                      background:
                        pct < 0.25 ? 'var(--accent-2)' : 'var(--accent)',
                    }}
                  />
                </div>
